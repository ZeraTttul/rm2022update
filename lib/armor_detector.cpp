...
void armor_detector :: selectLightbar(Mat frame,Mat binary,vector<armors>&armors_possible)
{
    Rgb rgb;
#ifdef BLUE
    ...
    for (size_t i = 0; i < contours.size(); i++) 
    {
        ...
        float angle = rec.size.width > rec.size.height ? rec.angle - 90 : rec.angle;
    ...
    vector<vector<bool>> is_armor(select_contours.size(),vector<bool>(select_contours.size(),true));
    for (int i = 0; i < select_contours.size(); i++) 
    {
        ...
        for (int j = i + 1; j < select_contours.size(); j++) 
        {
            ...
            if (diff >10 && diff < 70) {   //原来是10到70
                is_armor[i][j]==false;
                continue;
            }
            float Diff=abs(rect.center.y-rectA.center.y);
            if(Diff>30)
            {
                is_armor[i][j]==false;
                continue;
...
void armor_detector::selectrightarmor(vector<armors>&armors_possible, vector<armors>&armors,Mat binary)
{
    std::vector<bool> is_armor(armors_possible.size(), true);
    hh=0;
    int kkk=-1;
    for(int mark=0;mark<hi;mark++)
    { 
        ...
        cv::Point2f left_light = Point2f(x1-x4, y1-y4);
        cv::Point2f right_light = Point2f(x2-x3, y2-y3);
        cv::Point2f center_diff = armors_possible[mark].l.center - armors_possible[mark].r.center;
        ...
        float height = fmax(armors_possible[mark].l.size.width, armors_possible[mark].l.size.height);
        Rect re= Rect(armors_possible[mark].l.center.x,armors_possible[mark].l.center.y - height / 2, armors_possible[mark].r.center.x - armors_possible[mark].l.center.x, height);
        int armor_area = re.size().area();
        ...
        armors_possible[mark].boardh = (boardh_left + boardh_right) / 2;
        float board_width = abs(armors_possible[mark].l.center.x - armors_possible[mark].r.center.x);
        float board_height = (max(armors_possible[mark].l.size.width, armors_possible[mark].l.size.height) + max(armors_possible[mark].r.size.width, armors_possible[mark].r.size.height)) / 2;

        armors_possible[mark].board_ratio = armors_possible[mark].boardw/armors_possible[mark].boardh;
        if(armors_possible[mark].board_ratio>2.5)is_armor[mark]=false;
...
void armor_detector::selectfinalarmor(armors &final_armor,vector<armors>&armors,Mat binary)
{
     maxh=100;
     t=-1;
    for(int i=0;i<armors.size();i++)
    {
        if(armors[i].angle_diff<maxh)
        {
            maxh=armors[i].angle_diff;
            t=i;
        }
    }
    if(t==-1)return;
    final_armor = armors[t];
