...
/**
 * @description: 筛选灯条并组合 然后筛选出可能的装甲板
 * @param {Mat} frame
 * @param {Mat} binary
...
void armor_detector :: selectLightbar(Mat frame,Mat binary,vector<armors>&armors_possible)
{
    ...
    imshow("double", frame);
#endif
    vector<vector<Point> > contours;               //画出轮廓
    vector<vector<Point> > select_contours;        //筛选出的正确的灯条轮廓
    findContours(frame,
                 contours,
                 RETR_LIST,
                 CHAIN_APPROX_NONE);
    ...
    hi=-1;                                          //可能的装甲板的数量 视觉祖传的hi 没有特殊原因不准修改！
...
void armor_detector::selectrightarmor(vector<armors>&armors_possible, vector<armors>&armors,Mat binary)
{
    ...
    { 
        ...
        Rect re= Rect(armors_possible[mark].l.center.x,armors_possible[mark].l.center.y - height / 2, 
        ...
        float board_height = (max(armors_possible[mark].l.size.width, 
                                armors_possible[mark].l.size.height) + max(armors_possible[mark].r.size.width,
